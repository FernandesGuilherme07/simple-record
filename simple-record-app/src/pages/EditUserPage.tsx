...
const EditUserPage: React.FC = observer(() => {
  ...
  useEffect(() => {
    userStore.getPersonById(userId);
  }, [userStore, userId]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (userStore.isFormValid()) {
        await userStore.updatePerson(userId);
        history('/');
      } else {
        toast.warning('Por favor, preencha todos os campos obrigatórios antes de atualizar o usuário.');
      }
    } catch (error) {
